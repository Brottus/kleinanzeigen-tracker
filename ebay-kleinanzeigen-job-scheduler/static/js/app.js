// Kleinanzeigen Job Scheduler - Main JavaScript

// Internationalization (i18n)
const translations = {
    en: {
        appTitle: '📋 Kleinanzeigen Job Scheduler',
        logout: 'Logout',
...
async function showCreateJobModal() {
    document.getElementById('jobModalTitle').textContent = translations[currentLanguage].createNewJob;
    document.getElementById('jobForm').reset();
    document.getElementById('jobFormBtnText').textContent = translations[currentLanguage].createJob || 'Create Job';
...
async function handleJobSubmit(e) {
    ...
    try {
        ...
        alert('Error ' + (editingJobId ? 'updating' : 'creating') + ' job: ' + error.message);
    } finally {
...
async function editJob(jobId) {
    try {
        ...
        // Set editing mode
        editingJobId = jobId;
        document.getElementById('jobModalTitle').textContent = 'Edit Job: ' + job.name;
        document.getElementById('jobFormBtnText').textContent = 'Update Job';
...
async function toggleJob(jobId, newState) {
    try {
        ...
        alert('Error toggling job: ' + error.message);
    }
}

async function deleteJob(jobId, jobName) {
    const t = translations[currentLanguage];
    if (!confirm(`${t.delete} "${jobName}"?`)) return;
    ...
    form.innerHTML = Object.entries(config).map(([key, value]) => {
            ...
            return `
                <div class="form-group">
                    ...
                    <select id="config-${key}" data-key="${key}">
                        <option value="de" ${value.value === 'de' ? 'selected' : ''}>🇩🇪 Deutsch (German)</option>
                        <option value="en" ${value.value === 'en' ? 'selected' : ''}>🇬🇧 English</option>
                    </select>
                    <small style="color: var(--text-secondary);">${value.description || 'Language for notification field labels'}</small>
                </div>
            `;
        }
        ...
        return `
            <div class="form-group">
                ...
                <small style="color: var(--text-secondary);">${value.description || ''}</small>
            </div>
        `;
    }).join('');
}
...
async function saveConfig() {
    ...
    try {
        ...
        alert(translations[currentLanguage].saveChanges + ' ✓');
    } catch (error) {
        alert('Error: ' + error.message);
    }
}
...
function addUrlField(value = '') {
    ...
    fieldDiv.innerHTML = `
        <input type="text" 
               class="url-input" 
               placeholder="/s-wohnzimmer/muenchen/tisch/k0c88l6411" 
               value="${value}"
               style="flex: 1;"
               required>
        <button type="button" 
...
async function handleChangePassword(e) {
    ...
    try {
        ...
        successDiv.textContent = '✅ ' + response.message + ' Redirecting to login...';
